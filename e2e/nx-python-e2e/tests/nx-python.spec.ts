...
describe('nx-python e2e', () => {
  it('should create nx-python project', async () => {
    const app1 = 'app1';
    const lib1 = 'lib1';
    ensureNxProject('@skerna/python', 'dist/packages/nx-python');

    const nxJson = readJson('nx.json');
    nxJson.plugins = ['@skerna/python'];

    updateFile('nx.json', JSON.stringify(nxJson, null, 4));

    await runNxCommandAsync(
      `generate @skerna/python:project ${app1} --type "application" --packageName ${app1} --description ${app1}`
    );

    await runNxCommandAsync(
      `generate @skerna/python:project ${lib1} --type "library"  --packageName ${lib1} --description ${lib1}`
    );
  ...
  it('should create nx-python project with 3 levels', async () => {
    ...
    await runNxCommandAsync(
      `generate @skerna/python:project ${lib2} --type "library"  --packageName ${lib2} --description ${lib2}`
    );
