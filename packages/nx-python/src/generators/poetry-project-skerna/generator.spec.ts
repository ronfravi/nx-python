...
describe('application generator', () => {
  ...
  describe('individual package', () => {
    ...
    it('should run successfully with ruff linter', async () => {
      ...
      assertGeneratedFilesBase(appTree, 'apps/test', 'test');
      assertGeneratedFilesRuff(appTree, 'apps/test');
    });
...
function assertGeneratedFilesRuff(appTree: Tree, projectDirectory: string) {
  expect(appTree.exists(`${projectDirectory}/ruff.toml`)).toBeTruthy();
  expect(
    appTree.read(`${projectDirectory}/ruff.toml`, 'utf-8')
  ).toMatchSnapshot();
}
