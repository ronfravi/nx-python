...
- **util:** add @skerna/util package with some shared functions ([#105](https://github.com/lucasvieirasilva/nx-plugins/issues/105)) ([4f65c9c](https://github.com/lucasvieirasilva/nx-plugins/commit/4f65c9cc9319cba89f9650d5deebdacbbfbb1369))
...
- Added license to `@skerna/python` package json.

## [15.0.0] - 2023-01-09

### BREAKING CHANGES

- Update `@skerna/python` to support `nx` version `^15.0.0`.

### Added

- Added `@skerna/python:migrate-to-shared-venv` executor to migrate to shared venv.
...
- Fixed `@skerna/python:flake8` to return error when the `pylint.txt` has more than 1 line.

## [1.9.0] - 2022-07-18

### Changed

- Modify `@skerna/python:add` executor to add the CLI option `--extras`.
...
- Modify `@skerna/python:add` executor to add the CLI option `--group`.
...
- Modify `@skerna/python:build` executor to pass workspace root to dependency graph functions.
...
- Modify `@skerna/python:project` generator to add unit tests `conftest.py` file.
...
- Modify `@skerna/python:project` generator to support module name as a parameter.

## [1.1.0] - 2022-07-05

### Changed

- Modify `@skerna/python:project` generator and all executors to support root `pyproject.toml`.

## [1.0.0] - 2022-07-05

### Added

- Added `@skerna/python:project` project generator.
- Added `@skerna/python:add` Poetry add wrapper executor.
- Added `@skerna/python:build` Poetry build wrapper executor.
- Added `@skerna/python:flake8` Flake8 wrapper executor.
- Added `@skerna/python:install` Poetry install wrapper executor.
- Added `@skerna/python:remove` Poetry remove wrapper executor.
- Added `@skerna/python:sls-deploy` Serverless Framework Deploy wrapper executor.
- Added `@skerna/python:sls-package` Serverless Framework Package wrapper executor.
- Added `@skerna/python:tox` Tox wrapper executor.
- Added `@skerna/python:update` Poetry update wrapper executor.
